...
- Why cardiff is anti-specialized: cardiff is more sensitive to surface register than toxicbert is to multi-label aggregation, so cardiff's diagonal is more vulnerable to the filter's regression-to-survivor-mean. This is a hypothesis the paper proposes but does not prove; the optional scaling experiment in `TODO_report.md` is the natural follow-up that would falsify it.
