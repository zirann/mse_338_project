...
**Scale the clean survivor set, not the number of epochs.**

Specifically: hold all `fast_filt_e1` knobs fixed and raise `top_k` from 32 to e.g. 96 with `apply_contamination_filter: true` and `epochs: 1`. This should expand the clean training set from ~10 rows to ~25-30 rows after filtering. The hypothesis:

- if `own_minus_transfer(toxicbert)` strengthens beyond +0.034 and distinct_2 stays near 0.45+, this confirms that more *clean* signal (rather than more passes over the same small set) is what amplifies style-level evaluator-specific specialization;
- if it does not strengthen, the asymmetry is bounded by the structural difference between the two evaluators on the Qwen3 distribution and cannot be increased by data scale alone.

This is the single-knob follow-up most likely to either confirm or falsify the propagation model implied by the headline result, and it isolates the question cleanly from the two failure modes already characterized. Out of scope for the current report cycle.
