...
def dpo_loss(
    ...
    policy_chosen_logps_full=None,
    ref_chosen_logps_full=None,
):
    ...
    failure mode of suppressing the preferred completion.

    DPOP penalty SUPPORT: by default the penalty is computed on the chosen
    log-probs passed as `policy_chosen_logps` / `ref_chosen_logps` (under SamPO
    these are the equal-count subset sums -- the "sampo_subset" behavior). When
    `policy_chosen_logps_full` and `ref_chosen_logps_full` are BOTH provided,
    the penalty is computed on those instead (the "full_chosen" decoupled
    objective): SamPO subsets still drive the preference margin, but the DPOP
    anchor preserves the full chosen completion. The preference margin always
    uses the (subset) chosen/rejected log-probs, so passing the full args does
    not change the comparison term. Returns (loss_scalar, stats_dict) where
    stats includes `dpop_penalty`.
    """
    import torch
    ...
    if policy_chosen_logps_full is not None and ref_chosen_logps_full is not None:
        dpop_pen = torch.relu(ref_chosen_logps_full - policy_chosen_logps_full)
    else:
...
def run_one_regularized_dpo_round(
    ...
    - `dpop_support`: "sampo_subset" (default; the DPOP anchor uses the same
      (subset) chosen log-prob that feeds the preference margin) or "full_chosen"
      (decoupled objective: SamPO subsets drive the margin, but the DPOP anchor
      is computed on the FULL chosen completion). For non-SamPO arms the two are
      identical because the "none"/"lennorm" aggregation already uses full
      support, so existing arm behavior is unchanged.
    ...
    if dpop_support not in ("sampo_subset", "full_chosen"):
        raise ValueError(f"dpop_support must be sampo_subset|full_chosen; got {dpop_support!r}")
    ...
    for _epoch in range(max(1, int(round(epochs)))):
        ...
        for start in range(0, len(pairs), batch_size):
            ...
            # Decoupled DPOP: compute the anchor on the FULL chosen completion
            # while the preference margin keeps the (subset) aggregated log-probs.
            if dpop_support == "full_chosen":
                pol_c_full = (ptl_c * mask_c).sum(dim=-1)
                ref_c_full = (rtl_c * mask_c).sum(dim=-1)
                loss_dpo, stats = dpo_loss(
                    pol_c_logp, pol_r_logp, ref_c_logp, ref_r_logp, beta,
                    dpop_lambda=dpop_lambda,
                    policy_chosen_logps_full=pol_c_full,
                    ref_chosen_logps_full=ref_c_full,
                )
            else:
                loss_dpo, stats = dpo_loss(
    ...
    metadata = {
        ...
        "dpop_lambda": dpop_lambda,
        "dpop_support": dpop_support,
        "dpop_penalty_support": dpop_support,
        "seed": seed,
